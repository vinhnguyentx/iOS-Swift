//
//  BeerHistoryTableViewController.swift
//  HopJocky
//
//  Created by Matt Union on 4/30/16.
...
class BeerHistoryTableViewController: UITableViewController, UISearchResultsUpdating {
    var beers = [Beer]()
    var filteredBeers = [Beer]()
    var userRating: Dictionary<String, Int> = [:]
            ...
            if let snapshots = snapshot.children.allObjects as? [FDataSnapshot] {
                for snap in snapshots{
                    ...
                    if let beerDict = snap.value as? Dictionary<String, AnyObject>{
                        let name = snap.key
                        //if user has had this beer, add to beers list
                        if(user.beersHad[name] != nil){
                            let beer = Beer(key: name, dict: beerDict)
                            //add user's rating to the newly created beer object
                            beer.setUsrRating(user.beersHad[name]!)
...
//for sorting
extension BeerHistoryTableViewController: UISearchBarDelegate {
