//
//  BarHistoryTableViewController.swift
//  HopJocky
//
//  Created by Matt Union on 4/8/16.
//  Copyright © 2016 cs378. All rights reserved.

import UIKit
import Firebase

class BarHistoryTableViewController: UITableViewController, UISearchResultsUpdating {
    var bars = [Bar]()
            ...
            if let snapshots = snapshot.children.allObjects as? [FDataSnapshot] {
                for snap in snapshots{
                    ...
                    if let barDict = snap.value as? Dictionary<String, AnyObject>{
                        let name = snap.key
                        //if bar has been visited by user, then add to array
                        if user.barsVisited[name] != nil{
